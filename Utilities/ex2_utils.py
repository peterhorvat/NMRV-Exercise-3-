...
from Utilities.ex1_utils import gausssmooth
...
def generate_responses_2(width = 100, height = 100):
    responses = np.zeros((width, height), dtype=np.float32)
    wr = random.uniform(0.2, 0.5)
    hr = random.uniform(0.2, 0.5)
    responses[width-int(width*wr), height-int(width*hr)] = 1
    responses[height-int(width*hr), width-int(width*wr)] = 0.5
    return gausssmooth(responses, 10)
